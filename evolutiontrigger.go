...
func (s *EvolutionTrigger) EvolutionTriggerList(ctx context.Context, request operations.EvolutionTriggerListRequest) (*operations.EvolutionTriggerListResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "evolution-trigger_list"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := url.JoinPath(baseURL, "/api/v2/evolution-trigger/")
	if err != nil {
	...
	res := &operations.EvolutionTriggerListResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out operations.EvolutionTriggerListResponseBody
...
func (s *EvolutionTrigger) EvolutionTriggerRead(ctx context.Context, request operations.EvolutionTriggerReadRequest) (*operations.EvolutionTriggerReadResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "evolution-trigger_read"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v2/evolution-trigger/{id}/", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.EvolutionTriggerReadResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: contentType,
		RawResponse: httpRes,
	}
	...
	case httpRes.StatusCode == 200:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out shared.EvolutionTrigger
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.EvolutionTrigger = &out
		default:
			return nil, sdkerrors.NewSDKError(fmt.Sprintf("unknown content-type received: %s", contentType), httpRes.StatusCode, string(rawBody), httpRes)
		}
	case httpRes.StatusCode >= 400 && httpRes.StatusCode < 500:
