...
func (s *Location) LocationList(ctx context.Context, request operations.LocationListRequest) (*operations.LocationListResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "location_list"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := url.JoinPath(baseURL, "/api/v2/location/")
	if err != nil {
	...
	res := &operations.LocationListResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out operations.LocationListResponseBody
...
func (s *Location) LocationRead(ctx context.Context, request operations.LocationReadRequest) (*operations.LocationReadResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "location_read"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v2/location/{id}/", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.LocationReadResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: contentType,
		RawResponse: httpRes,
	}
