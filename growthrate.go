...
func (s *GrowthRate) GrowthRateList(ctx context.Context, request operations.GrowthRateListRequest) (*operations.GrowthRateListResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "growth-rate_list"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := url.JoinPath(baseURL, "/api/v2/growth-rate/")
	if err != nil {
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out operations.GrowthRateListResponseBody
...
func (s *GrowthRate) GrowthRateRead(ctx context.Context, request operations.GrowthRateReadRequest) (*operations.GrowthRateReadResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "growth-rate_read"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v2/growth-rate/{id}/", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
