...
func (s *PalParkArea) PalParkAreaList(ctx context.Context, request operations.PalParkAreaListRequest) (*operations.PalParkAreaListResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "pal-park-area_list"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := url.JoinPath(baseURL, "/api/v2/pal-park-area/")
	if err != nil {
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out operations.PalParkAreaListResponseBody
...
func (s *PalParkArea) PalParkAreaRead(ctx context.Context, request operations.PalParkAreaReadRequest) (*operations.PalParkAreaReadResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "pal-park-area_read"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v2/pal-park-area/{id}/", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
