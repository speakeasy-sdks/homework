...
func (s *PokemonSpecies) PokemonSpeciesList(ctx context.Context, request operations.PokemonSpeciesListRequest) (*operations.PokemonSpeciesListResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "pokemon-species_list"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := url.JoinPath(baseURL, "/api/v2/pokemon-species/")
	if err != nil {
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out operations.PokemonSpeciesListResponseBody
...
func (s *PokemonSpecies) PokemonSpeciesRead(ctx context.Context, request operations.PokemonSpeciesReadRequest) (*operations.PokemonSpeciesReadResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "pokemon-species_read"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v2/pokemon-species/{id}/", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	case httpRes.StatusCode == 200:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out shared.PokemonSpeciesInput
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
