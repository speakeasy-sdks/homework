...
func (s *Generation) GenerationList(ctx context.Context, request operations.GenerationListRequest) (*operations.GenerationListResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "generation_list"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := url.JoinPath(baseURL, "/api/v2/generation/")
	if err != nil {
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out operations.GenerationListResponseBody
...
func (s *Generation) GenerationRead(ctx context.Context, request operations.GenerationReadRequest) (*operations.GenerationReadResponse, error) {
	hookCtx := hooks.HookContext{OperationID: "generation_read"}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := utils.GenerateURL(ctx, baseURL, "/api/v2/generation/{id}/", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
